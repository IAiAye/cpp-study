...
int main()
{
    ...
    emp_list.add_employee(new Manager("정준하", 42, "과장", 4, 13));
...
// Employee* 가 Manger를 가르켜도 문제없이 작동이 되지만, 멤버 함수를 호출할 경우 무조건 기반 클래스의 함수가 출력되는 것을 알 수 있다.
// 이때 virtual 과 override 키워드를 추가하여 문제를 해결할 수 있다.